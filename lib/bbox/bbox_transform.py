...
def nonlinear_transform(ex_rois, gt_rois):
    ...
    # leonid: adding these tests for debugging of "RuntimeWarning: invalid value encountered in log"
    if np.any(gt_widths<=0) or np.any(gt_heights<=0) or np.any(ex_widths<=0) or np.any(ex_heights<=0):
        print('Bad values: gt_widths={0}, gt_heights={1}, ex_widths={2}, ex_heights={3}'.format(
            np.any(gt_widths <= 0),
            np.any(gt_heights <= 0),
            np.any(ex_widths <= 0),
            np.any(ex_heights <= 0)
...
def nonlinear_pred(boxes, box_deltas):
    ...
    # Leonid: adding protection against overflow
    dw = np.minimum(dw, 10)
    dh = np.minimum(dh, 10)
