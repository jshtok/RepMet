...
from utils.image import get_image, get_image_scene,get_image_m, tensor_vstack
...
def get_rpn_testbatch(roidb, cfg):
    ...
    imgs, roidb, _ = get_image(roidb, cfg)
    im_array = imgs
...
def get_rpn_batch(roidb, cfg):
    ...
    if roidb[0]['gt_classes'].size > 0:
        ...
        if 'clsIds2use' in cfg.dataset:
            gt_boxes = gt_boxes[[ix for ix in range(gt_boxes.shape[0]) if gt_boxes[ix, 4] in cfg.dataset.clsIds2use]]
    else:
...
def get_rpn_batch_scene2(scenedb, cfg):
    """
    prototype for rpn batch: data, im_info, gt_boxes
    :param roidb: ['image', 'flipped'] + ['gt_boxes', 'boxes', 'gt_classes']
    :return: data, label
    """
    assert len(scenedb) == 1, 'Single batch only'
    imgs, scenedb,img_names = get_image_m(scenedb, cfg)
    im_array = imgs[0]
    img_fname = img_names[0]
    im_info = np.array([scenedb[0]['im_info']], dtype=np.float32)

    # gt boxes: (x1, y1, x2, y2, cls)
    if scenedb[0]['gt_classes'].size > 0:
        gt_inds = np.where(scenedb[0]['gt_classes'] != 0)[0]
        gt_boxes = np.empty((scenedb[0]['boxes_views']['top'].shape[0], 5), dtype=np.float32)
        gt_boxes[:, 0:4] = scenedb[0]['boxes_views']['top'][gt_inds, :]
        gt_boxes[:, 4] = scenedb[0]['gt_classes'][gt_inds]
...
def get_rpn_batch_scene(scenedb, cfg):
    """
    prototype for rpn batch: data, im_info, gt_boxes
    :param roidb: ['image', 'flipped'] + ['gt_boxes', 'boxes', 'gt_classes']
    :return: data, label
    """
    def rot_view(boxes,W,H):
        # input: np.array(n,4) = left, top, right, bottom
        boxes_out = np.zeros_like(boxes)
        boxes_out[:, 0] = boxes_out[:, 1] # Left = old Top
        boxes_out[:, 1] = W - boxes_out[:, 2]  # Top = Width - Right
        boxes_out[:, 2] = H - boxes_out[:, 3]  # Right = Height - Bottom
        boxes_out[:, 3] = boxes_out[:, 0]  # Bottom = Left
        return boxes_out

    def rot_90_counter(img):
        # rotate 90 degrees counterclockwise: transpose+ vertical flit
        img = cv2.flip(cv2.transpose(img[0].transpose()),flipCode=0).transpose()
        img = np.expand_dims(img, axis=0)
        return img

    def rot_im_info_view(iiv):
        iiv = [iiv[1],iiv[0],iiv[2]]
        return iiv

    assert len(scenedb) == 1, 'Single batch only'
    imgs, scenedb,img_names = get_image_scene(scenedb, cfg)
    im_array = imgs[0] #per-view dictionary
    img_fname = img_names[0]
    im_info_views = scenedb[0]['im_info_views']
    gt_boxes = {}
    # gt boxes: (x1, y1, x2, y2, cls)
    views_list = scenedb[0]['image_views'].keys()
    W = im_array['left'].shape[1]
    H = im_array['left'].shape[0]
    if scenedb[0]['gt_classes'].size > 0:
        gt_inds = np.where(scenedb[0]['gt_classes'] != 0)[0]
        view = 'top'
        gt_boxes[view] = np.empty((scenedb[0]['boxes_views'][view].shape[0], 5), dtype=np.float32)
        gt_boxes[view][:, 0:4] = scenedb[0]['boxes_views'][view][gt_inds, :]
        gt_boxes[view][:, 4] = scenedb[0]['gt_classes'][gt_inds]

        for view in ['left','right']: # rotate the boxes
            gt_boxes[view] = np.empty((scenedb[0]['boxes_views'][view].shape[0], 5), dtype=np.float32)
            gt_boxes[view][:, 0:4] = rot_view(scenedb[0]['boxes_views'][view][gt_inds, :],W,H)
            gt_boxes[view][:, 4] = scenedb[0]['gt_classes'][gt_inds]
        ...
        #leonid: filter boxes outside the set of supported categories
        if 'clsIds2use' in cfg.dataset:
            gt_boxes[view] = gt_boxes[view][[ix for ix in range(gt_boxes[view].shape[0]) if gt_boxes[view][ix, 4] in cfg.dataset.clsIds2use]]
    else:
        gt_boxes = np.empty((0, 5), dtype=np.float32)
    ...
    for view in ['top','left','right']:
        data['data'] = im_array[view]
        # data['data'] = np.concatenate((data['data'], rot_90_counter(im_array[view])), axis=0)
        if view == 'top':
            data['im_info_' + view] = np.array([im_info_views[view]], dtype=np.float32)
        else:
            data['im_info_' + view] = np.array(rot_im_info_view(im_info_views[view]), dtype=np.float32)

    with open(cfg.dataset.homog_data_fname, 'r') as fid:
        values = [x.strip() for x in fid.readlines()]
    homog = np.zeros((2,9),dtype=float)
    homog[0,:]= values[0:9]
    homog[1, :] = values[9:]


    #data['homog_data'] =homog
    # for view in views_list:
    #     data['data'] = np.concatenate((data['data'],im_array[view]),axis=0) if data['data'] is not None else im_array[view]
    #     data['im_info_'+view] = np.array(im_info_views[view], dtype=np.float32)
        #label['gt_boxes_'+view] = gt_boxes[view]
    label['gt_boxes'] = gt_boxes['top']
    return data, label, img_fname
