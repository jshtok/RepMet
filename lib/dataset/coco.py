...
class coco(IMDB):
    def __init__(self, image_set, root_path, data_path, result_path=None, mask_size=-1, binary_thresh=None, categ_index_offs=0, per_category_epoch_max=0,classes_list_fname='',num_ex_per_class=0):
        """
        ...
        classes_fname = '/dccstor/leonidka1/data/coco/train_cats_split.txt'
        with open(classes_fname, 'r') as fid:
            self.test_class_list = [x.strip() for x in fid.readlines()]
    ...
    def _write_coco_results(self, all_boxes, res_file, ann_type, all_masks):
        ...
        if ann_type == 'bbox':
            ...
            # for cls_ind, cls in enumerate(self.classes) if cls in self.test_class_list and not cls == '__background__']
        elif ann_type == 'segm':
            data_pack = [{'cat_id': self._class_to_coco_ind[cls],
                          'cls_ind': cls_ind,
                          'cls': cls,
