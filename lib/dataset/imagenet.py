...
class ImageNet(IMDB):
    def __init__(self, image_set, root_path, devkit_path, result_path=None, mask_size=-1,\
                 binary_thresh=None, categ_index_offs=0, per_category_epoch_max=0, ds_main_name='CLS-LOC',classes_list_fname='',num_ex_per_class=''):
        """
        ...
        self.ds_main_name = ds_main_name
        sis = image_set.split(':')
        if len(sis) > 1:
            image_set = sis[0]
            self.ds_main_name = sis[1]
        ...
        super(ImageNet, self).__init__('imagenet_' + modifier, image_set, root_path, devkit_path, result_path)  # set self.name

        self.image_set_type = image_set.split('_')[0]

        # self.year = year
        self.root_path = root_path
        self.devkit_path = os.path.join(devkit_path, 'devkit')
        self.data_path = devkit_path

        synsets = sio.loadmat(os.path.join(self.devkit_path, 'data', 'meta_'+base_modifier+'.mat'))
        if categ_index_offs==0:
            self.classes = ['__background__']
            self._wnid = [0]
            self.num_classes = len(synsets['synsets'][0]) + 1  # N + background
            for i in xrange(self.num_classes - 1):
                self.classes = self.classes + [synsets['synsets'][0][i][2][0]]
                self._wnid = self._wnid + [synsets['synsets'][0][i][1][0]]
        else:
            self.classes=[]
            self._wnid = []
            self.num_classes = len(synsets['synsets'][0])+categ_index_offs  # N + # previous classes from other datasets
            for i in xrange(self.num_classes - categ_index_offs):
                self.classes = self.classes + [synsets['synsets'][0][i][2][0]]
                self._wnid = self._wnid + [synsets['synsets'][0][i][1][0]]

        self._wnid_to_ind = dict(zip(self._wnid, xrange(categ_index_offs,self.num_classes)))
        # wnid_to_ind keys are [0, u'n04509417', u'n03642806', u'n02398521', u'n04540053', u'n04154565', u'n03394916', u'n04517823', u'n03494278', u'n02342885']
        # wnid_to_ind values are [0,190,101, ...]
        self._class_to_ind = dict(zip(self.classes, xrange(categ_index_offs,self.num_classes)))
    ...
    def load_image_set_index(self):
        ...
        :return:
        """
        image_set_index_file = os.path.join(self.data_path, 'ImageSets', self.ds_main_name, self.image_set + '.txt')
    ...
    def image_path_from_index(self, index):
        ...
        """
        image_file = os.path.join(self.data_path, 'Data', self.ds_main_name, self.image_set_type, index + '.JPEG')
    ...
    def load_imagenet_annotation(self, index):
        ...
        filename = os.path.join(self.data_path, 'Annotations', self.ds_main_name, self.image_set_type, index + '.xml')
    ...
    def do_python_eval(self):
        ...
        info_str = ''
        # annopath = os.path.join(self.data_path, 'Annotations','CLS-LOC',self.image_set, '{0!s}.xml')
        annopath = os.path.join(self.data_path, 'Annotations', 'CLS-LOC', 'val', '{0!s}.xml') #TODO: remove - temporary fix to run val_partial
        imageset_file = os.path.join(self.data_path, 'ImageSets', 'CLS-LOC', self.image_set + '.txt')
