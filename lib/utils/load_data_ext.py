...
def load_gt_roidb(dataset_name, image_set_name, root_path, dataset_path, result_path=None,
                  flip=False, categ_index_offs=0, per_category_epoch_max=0, database_csv='',classes_list_fname='',\
                  return_num_classes=False, return_imdb=False,num_ex_per_class=0, force_reload = False):
    """ load ground truth roidb """
    imdb = eval(dataset_name)(image_set_name, root_path, dataset_path, result_path, categ_index_offs=categ_index_offs,\
                              per_category_epoch_max=per_category_epoch_max,database_csv=database_csv,classes_list_fname=classes_list_fname,num_ex_per_class=num_ex_per_class)
    roidb = imdb.gt_roidb(force_reload)
    if flip:
        roidb = imdb.append_flipped_images(roidb)
    if return_num_classes:
        if return_imdb:
            return roidb, imdb.num_classes, imdb
        else:
            return roidb, imdb.num_classes
    else:
        return roidb
